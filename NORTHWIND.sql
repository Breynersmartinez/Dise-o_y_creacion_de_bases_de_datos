...
select min(unit_price) as valor_minimo,
       ...
       round(avg(unit_price)::numeric, 4) as valor_promedio_round
from products;

-- cuáles son los productos que la empresa tiene en bodega con las siguientes cantidades: 120, 22, 24, 35, 42, 9
select product_name, units_in_stock
from products
where units_in_stock in (120, 22, 24, 35, 42, 9);

-- cuántos productos tiene en bodega
select sum(units_in_stock) from products;
-- o
select sum(units_in_stock) as productos_en_bodega from products;

-- cuáles productos inician con la letra s
select *
from products
where product_name like 's%';

-- cuáles productos terminan con la letra n?
select *
from products
where product_name like '%n';

-- cuáles productos contienen la frase cho?
select *
from products
where product_name like '%cho%';

-- cuáles son los códigos de las órdenes y el valor del flete que fueron registrados por el cliente victé entre el 1 de enero de 1996 y el 31 de diciembre de 1997
select customer_id, order_date, freight, order_id
from orders
where customer_id = 'VICTE'
  and order_date between '1996-01-01' and '1997-12-31';

-- buscar empleados de londres
select * from employees;
select last_name, first_name, city
from employees
where lower(city) = 'london';

-- cuáles son los empleados que tienen como ciudad: londres, tacoma y seattle
select last_name, first_name, city
from employees
where city in ('London', 'Tacoma', 'Seattle');

-- productos que tienen un costo unitario entre 20 y 40 dólares
select product_name, unit_price
from products
...
-- contar productos por categorias con precio unitario >20
-- pero solo para categorias que tienen al meenos 5 de essos

select category_id, count(*) as productos_caros_contados
from products
where unit_price > 20 -- elimina los productos con precio unitario menos a 20
group by category_id
having count(*)>= 6;

-- agrupar por cargo
select title, count(*) "TOTAL EMPLEADOS"
from employees
group by title;

-- agruparempleados que viven en Seattle
select title, count(*) "TOTAL EMPLEADOS"
from employees
where city = 'Seattle'
group by title;
...
-- cuantas ordenes ah realizado cada empleado en a empresa
-- desplegar el nombre del empleado concatenado cn el apellido y el codigo y en frente la cantidad de ordenes que ah relizado
